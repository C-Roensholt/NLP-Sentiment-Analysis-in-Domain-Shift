...
def plotting(df):
    ...
    ax.plot(df[['train_loss', 'val_loss']], label=['Train loss', 'Validation loss'])
    ax.legend()
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
...
def runNN(
    ...
    if not use_trained:
        ...
        results = net.learn(learning_rate,momentum,num_epochs,dump_trained)
        plotting(results)
...
def main():
    train = load_train_handcrafted()[:1000]
    dev = load_dev_handcrafted()[:1000]
    test = load_hard_handcrafted()[:1000]
